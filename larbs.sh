...
manualinstall() {
	...
	sudo -u "$name" git clone -C "$repodir/$1" --depth 1 --single-branch \
		--no-tags -q "https://aur.archlinux.org/$1.git" "$repodir/$1" \
		|| { cd "$repodir/$1" || return 1 ; sudo -u "$name" git pull
			--force origin master ;}
	cd "$repodir/$1" || exit 1
...
putgitrepo() {
	...
	sudo -u "$name" git clone -C "$repodir/$1" \
	--depth 1 --single-branch --no-tags -q --recursive \
	-b "$branch" --recurse-submodules "$1" "$dir"
